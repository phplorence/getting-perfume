...
class Helper extends Controller
{
    ...
    function resize_image($method,$image_loc,$new_loc,$width,$height) {
        ...
        if (count($GLOBALS['errors']) > 0) { self::echo_errors(); return false; }
        ...
        if (count($GLOBALS['errors']) > 0) { @imagedestroy($image); self::echo_errors(); return false; }

        if ($method == 'force') { $new_image = self::resize_image_force($image,$width,$height); }
        elseif ($method == 'max') { $new_image = self::resize_image_max($image,$width,$height); }
        elseif ($method == 'crop') { $new_image = self::resize_image_crop($image,$width,$height); }
        ...
        if (count($GLOBALS['errors']) > 0) { @imagedestroy($image); @imagedestroy($new_image); self::echo_errors(); return false; }
